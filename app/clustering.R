...
observe({
    updateSelectInput(session, "PCAColor", choices=SubmitReactive()$groups, selected='SampleName')
    updateSelectInput(session, "PCAShape", choices=c("None",SubmitReactive()$groups), selected='None')
})

pcaplotter <- reactive({
    p <- pca(DESeqReactive()$ddscounts, metadata=SubmitReactive()$metadata, removeVar=0.1)
    if (input$PCAShape=="None"){
        pcaplot <- biplot(p, colby=input$PCAColor, 
            legendPosition=input$PCALegendPos, legendLabSize=input$PCALegendLabel, ellipse=input$Ellipsoid, legendIconSize=input$PCALegendIcon, pointSize=input$PCAPointSize, labSize=input$PCALabelSize)
    } else {
...
distanceplotter <- reactive({
    sampleDists <- as.matrix(dist(t(assay(DESeqReactive()$rld))))

    metatemp <- ProjectData$metadata
    anno_cols <- which(names(metatemp)==input$PCAColor)
...
############################################################################
observe({
    updateSelectizeInput(session, "EigenMetavars", choices=SubmitReactive()$groups)
})
